...
void thread_fs_script(void *arg)
{
    ...
    while (fgets(line_buffer, 1024, fd_script) != NULL) {
        ...
        } while (command_index < total_command_parts &&
        ...
        if (strcmp(command, "MOUNT") == 0) {
            ...
            if (strcmp(data_source, "DATA") == 0) {
                ...
                data =
                    mmap(NULL, data_size, PROT_READ, MAP_PRIVATE, data_fd, 0);
            } else {
                data = NULL;
                data_size = 0;
            }
...
static struct {
    const char *name;
    void (*func)(void *);
} commands[] = { { "info", thread_fs_info },    { "ls", thread_fs_ls },
                 { "add", thread_fs_add },      { "rm", thread_fs_rm },
                 { "cat", thread_fs_cat },      { "stat", thread_fs_stat },
