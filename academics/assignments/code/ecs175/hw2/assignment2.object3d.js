...
class Object3D
{
    ...
    createVAO( gl, shader )
    {
        ...
        let pos_loc = shader.getAttributeLocation("a_position")
        let norm_loc = shader.getAttributeLocation("a_normal")

        let has_normals = (this.vertices.length !== this.indices.length)
        
        const STRIDE = has_normals ? 6 * Float32Array.BYTES_PER_ELEMENT : 0

        if (pos_loc !== -1) {
            gl.enableVertexAttribArray(pos_loc)
            gl.vertexAttribPointer(pos_loc, 3, gl.FLOAT, false, STRIDE, 0)
        }
        if (norm_loc !== -1 && has_normals) {
            gl.enableVertexAttribArray(norm_loc)
            gl.vertexAttribPointer(norm_loc, 3, gl.FLOAT, false, STRIDE, 12)
        }
