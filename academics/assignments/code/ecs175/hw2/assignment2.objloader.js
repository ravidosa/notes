...
class OBJLoader {
    ...
    load() {
        ...
        for (let i = 0; i < vertex_positions.length / 3; i++) {
            for (let j = 0; j < 3; j++) {
                vertex_data.push(vertex_positions[i * 3 + j])
            }
            for (let j = 0; j < 3; j++) {
                vertex_data.push(vertex_normals[i * 3 + j])
            }
        }
    ...
    resolveIndexPairs(vertex_positions, vertex_normals, position_indices, normal_indices) 
    {
        ...
        for (let i = 0; i < num_entries; i++) {
            ...
            if (out_vertex_map.has(key)) {
                out_indices.push(out_vertex_map.get(key))
            }
            else {
                let idx = out_vertex_positions.length / 3
                for (let j = 0; j < 3; j++) {
                    out_vertex_positions.push(vertex_positions[position_idx + j])
                    out_vertex_normals.push(vertex_normals[normal_idx + j])
                }
                out_vertex_map.set(key, idx)
    ...
    {
        let coords = face_string.trim().split(/\s+/)
        let indices = []

        for (let i = 1; i < coords.length; i++) {
            let parts = coords[i].split("/")
            let idx = parseInt(parts[entry_index]) - 1
            indices.push(idx)
        }
