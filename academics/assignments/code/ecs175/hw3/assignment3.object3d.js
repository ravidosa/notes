...
class ShadedObject3D extends Object3D { 
    ...
    createVAO( gl, shader )
    {
        ...
        let stride = num_total_components * FLOAT_SIZE, offset = 0

        let location = shader.getAttributeLocation( 'a_position' )
        if (location >= 0) {
            gl.enableVertexAttribArray(location)
            gl.vertexAttribPointer(location, this.num_components_vec3, gl.FLOAT, false, stride, 0 * FLOAT_SIZE)
        }

        location = shader.getAttributeLocation( 'a_normal' )
        if (location >= 0) {
            gl.enableVertexAttribArray(location)
            gl.vertexAttribPointer(location, this.num_components_vec3, gl.FLOAT, false, stride, 3 * FLOAT_SIZE)
        }

        location = shader.getAttributeLocation( 'a_tangent' )
        if (location >= 0 && this.material.hasTexture()) {
            gl.enableVertexAttribArray(location)
            gl.vertexAttribPointer(location, this.num_components_vec3, gl.FLOAT, false, stride, 6 * FLOAT_SIZE)
        }

        location = shader.getAttributeLocation( 'a_texture_coord' )
        if (location >= 0 && this.material.hasTexture()) {
            gl.enableVertexAttribArray(location)
            gl.vertexAttribPointer(location, this.num_components_vec2, gl.FLOAT, false, stride, 9 * FLOAT_SIZE)
        }
