...
import os
from utils import norm, energy, V_inf_pot_pet
...
y = norm(x, y)
E = energy(x, y, V_inf_pot_pet, (B))
print(E)
plt.ylim((-1, 1.5))
plt.plot(x, V_inf_pot_pet(x, (B)), "k:")
...
for j, (delta_y, M) in enumerate(ann):
    ...
    for _ in range(M):
        ...
        y_ = norm(x, y_)
        E_ = energy(x, y_, V_inf_pot_pet, (B))
        if E_ < E:
            y, E = y_, E_
...
exp = np.pi ** 2 / 4 - 8 / (3 * np.pi)
print("expected E:", exp, round(abs(exp - E) / E * 100, 2), "% error")
plt.xlabel("x")
plt.ylabel("y")
plt.title("Variational Method (perturbed infinite square well)")
plt.legend()
