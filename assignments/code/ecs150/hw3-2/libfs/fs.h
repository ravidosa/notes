 ...
 *
 * The function implicitely increments the file descriptor's offset by the
 ...
 * is at the end of the file). The function implicitely increments the file
 * descriptor's offset by the number of bytes that were actually read.
 *
 * Return: -1 if no FS is currently mounted, or if file descriptor @fd is
 * invalid (out of bounds or not currently open), or if @buf is NULL. Otherwise
