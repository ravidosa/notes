...
int main(void) {
    ...
    scanf("%lf %lf",&L_x,&L_y);
    printf("\nEnter coefficient of potential at x=0, x=L_x, y=0, y=L_y \n");
    ...
    for (x = 0; x < xx; x = x + 1) {
        for (y = 0; y < yy; y = y + 1) {
            V_curr[x][y] = initial_cond(x, y, xx, yy, dx, dy, V_0);
        }
    }
...
double initial_cond(int x, int y, int xx, int yy, double dx, double dy, double V_0[2][2])
{
    if (0 < x && x < xx & y == yy - 1) {
        return V_0[1][1]*x*dx;
    }
