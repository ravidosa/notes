...
import { SHADER_MAX_LIGHTS, hex2rgb, json2transform } from './utils.js'
...
class Scene {
     ...
     */
    constructor(scene_config, gl, light_shader) {
        ...
        this.scenegraph = this.loadScenegraphNode(scene_config.scenegraph, gl, light_shader)
    ...
    setShader( gl, shader )
    {
        ...
        for (let node of this.getNodes()) {
            if (node.type == 'model') {
                node.setShader(gl, shader)
            }
            if (node.type == 'light') {
                node.setTargetShader(shader)
            }
        }
    }
    ...
    loadModels( models_config, gl )
    {
        ...
        for (let model_config of models_config) {
            let loader = new OBJLoader(model_config.obj) // Load the OBJ file
    ...
    loadLights( lights_config ) {
        ...
        for (let light_config of lights_config) {
            lights[light_config.name] = light_config // Load the light
        }
    ...
    instantiateModel( name, gl ) 
    {
        ...
        return new ShadedObject3D( gl, vertices, indices, gl.TRIANGLES, material )
    }
     ...
     */
    instantiateLight( name, gl, light_shader )
    {
        if (!(name in this.lights)) {
            throw new Error(`Unable to find light "${name}" requested by scenegraph`)
        }
        ...
        switch(light.type) {
            case 'ambient':
                return new AmbientLight( this.light_counts[this.lights[name].type], color, light.intensity, null, gl, light_shader )

            case 'point':
                return new PointLight( this.light_counts[this.lights[name].type], color, light.intensity, null, gl, light_shader )

            case 'directional':
                return new DirectionalLight( this.light_counts[this.lights[name].type], color, light.intensity, null, gl, light_shader )
     ...
     */
    loadScenegraphNode( node_config, gl, light_shader ) {
        let node = null
        ...
        switch(node_config.type) {
            ...
            case 'model': // Geometry node containing a model
                node = new ModelNode(
                    this.instantiateModel(node_config.content, gl), // Field "content" refers to the model to be associated with this node
                    node_config.name,
                    node_config.type,
                    'transformation' in node_config ? json2transform(node_config.transformation) : mat4.create()
                )
            ...
            case 'light': // Light node containing a light
                node = new LightNode(
                    this.instantiateLight(node_config.content, gl, light_shader), // Field "content" refers to the light to be associated with this node
                    node_config.name,
                    node_config.type,
                    'transformation' in node_config ? json2transform(node_config.transformation) : mat4.create()
                )
        ...
        if ('children' in node_config) {
            for (let child of node_config.children) {
                ...
                let child_node = this.loadScenegraphNode(child, gl, light_shader)
     ...
     * @param {WebGL2RenderingContext} gl The webgl2 rendering context
     */
    render( gl, excludes = null ) {
        this.scenegraph.render( gl, excludes )
    }
}
...
class ModelNode extends SceneNode {
     ...
     */
    render( gl, excludes = null ) {
        if (!excludes || !excludes.includes(this.type)) {
            this.obj3d.render( gl )
        }
        super.render( gl, excludes )
    }
}
