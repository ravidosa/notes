...
class AppState
{
    ...
    constructor( )
    {
        ...
        this.ui_categories = {
            ...
            'Shading Debug': {
                ...
                'Preview': document.getElementById( 'shadingPreviewFBO'   ),
            },
            'Control': {
                'Camera':     document.getElementById( 'controlCamera'    ),
                'Scene Node': document.getElementById( 'controlSceneNode' )
            },
            'Select Filter':     document.getElementById( 'selectFilterSelect'    ),
        ...
        this.ui_categories['Select Filter'].onchange = () => {
            this.ui_state['Select Filter'] = +(this.ui_categories['Select Filter'].value)
        }
        ...
        this.ui_categories['3D Scene'].onchange = (evt) => {
            if (this.onOpen3DSceneCallback) {
                ...
                this.ui_categories['Select Scene Node'].value = 
                    this.ui_categories['Select Scene Node'].getElementsByTagName('option')[0].value
                this.ui_state['Select Scene Node'] = this.ui_categories['Select Scene Node'].value
            }
        }
    }
    ...
    update( )
    {
        ...
        if ( Input.isKeyDown( 'n' ) ) {
            ...
            this.updateUI( 'Shading Debug', 'Preview' )
        } else {
            this.updateUI( 'Shading Debug', '' )
        }
