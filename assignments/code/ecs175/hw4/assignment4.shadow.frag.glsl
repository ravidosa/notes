...
float computeVisibility(vec4 shadow_coord, sampler2D shadow_tex)
{
    ...
    bool outside = (proj.x < 0.0 || proj.x > 1.0 || proj.y < 0.0 || proj.y > 1.0 || proj.z < 0.0 || proj.z > 1.0);

    float shadow_map_depth = texture(shadow_tex, proj.xy).r;

    float bias = 0.005;

    bool visible = outside || (proj.z - bias <= shadow_map_depth);
    if (visible) {
        return 1.0;
    }
    else {
